...
export default function Wallet() {
  ...
  const load = async () => {
    ...
    setError(false);
    try {
      const data = await base44.entities.Ride.filter({ passenger_id: user.id, status: { $in: ["COMPLETED", "RATED", "PAYMENT_PENDING"] } }, "-created_date", 20);
      setRides(data);
    } catch { setError(true); } finally { setLoading(false); }
  };
  useEffect(() => { load(); }, [user?.id]);
  if (loading) return <LoadingScreen className="h-full" label="Cargando tu billetera..." />;
  return <PullToRefresh onRefresh={load}><div className="responsive-content mx-auto px-5 pt-6 pb-8 space-y-6">
