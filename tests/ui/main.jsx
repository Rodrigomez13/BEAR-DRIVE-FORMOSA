...
createRoot(document.getElementById('root')).render(<ThemeProvider><BrowserRouter><div className="p-4" hidden={window.location.pathname.startsWith("/passenger") || window.location.pathname.startsWith("/driver")}><p>Prueba local con datos simulados</p><nav className="flex gap-4"><Link to="/operations">Soporte</Link><Link to="/change">Cambio de destino</Link><Link to="/passenger/benefits">Deuda</Link></nav></div><Routes><Route path="/mobile/passenger" element={<PassengerShell />}><Route path="account" element={<AccountHome />} /></Route><Route path="/passenger" element={<PassengerShell />}><Route path="rides" element={<RideHistory />} /><Route path="account" element={<AccountHome />} /><Route path="account/personal" element={<PassengerProfile />} /><Route path="wallet" element={<Wallet />} /></Route><Route path="/driver" element={<DriverShell />}><Route path="rides" element={<RideHistory mode="driver" />} /><Route path="account" element={<AccountHome />} /><Route path="account/personal" element={<PassengerProfile />} /></Route><Route path="/payments" element={<div className="p-4"><AdminPayments/></div>}/><Route path="/operations" element={<div className="p-4"><AdminOperations/></div>}/><Route path="/change" element={<Change/>}/><Route path="*" element={<AccountDebtNotice><div>Beneficios</div></AccountDebtNotice>}/></Routes></BrowserRouter></ThemeProvider>);
